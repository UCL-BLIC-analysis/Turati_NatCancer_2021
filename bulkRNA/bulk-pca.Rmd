...
We transform the data all together with the VST transformation function. This essentially log-transcforms the counts while taking into account the library size.
...
### Treated vs untreated samples (all patients together)

This is the initial PCA is built with the `bulk4_dds` data set, using VST-transformed counts.
...
ggsave("output/figures/FigS5a_pca_3patients.pdf",
...
openxlsx::saveWorkbook(wb, "output/bulkRNAseq_data_FigS5a.xlsx", overwrite = TRUE)
```

### Treated vs untreated samples (PT1 only)
...
### Treated vs untreated samples (PT12 only)
...
### Treated vs untreated samples (PT13 only)
...
plots[[3]] +
  ...
  theme(legend.position = "top")
ggsave("output/figures/FigS5b_pca_headache.pdf",
...
openxlsx::saveWorkbook(wb, "output/bulkRNAseq_data_FigS5b.xlsx", overwrite = TRUE)

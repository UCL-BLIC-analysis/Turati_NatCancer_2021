...
params:
  ...
  min_median_reads: 10
  data_file: ""
title: "`r paste0('Copy number events in ', params$Patient, ' ', params$sample_type, 'using ', params$bin_size, ' bin size (Data processing)')`"
---
...
```{r data_processing.for.Pt1}
# Only for Pt1 PDXs
if (sample_type != "dia") {
  rawdata_data <- rawdata_data[ , -which(colnames(rawdata_data) %in% which_cells(rawdata_data, meta_data, Mouse = "Patient"))]
  segcopy_data <- segcopy_data[, -which(colnames(segcopy_data) %in% which_cells(segcopy_data, meta_data, Mouse = "Patient"))]
  segnorm_data <- segnorm_data[ , -which(colnames(segnorm_data) %in% which_cells(segnorm_data, meta_data, Mouse = "Patient"))]
  segfixed_data <-segfixed_data[ , -which(colnames(segfixed_data) %in% which_cells(segfixed_data, meta_data, Mouse = "Patient"))]
  segstats_data <- segstats_data[-which(rownames(segstats_data) %in% as.character(subset(meta_data, Mouse == "Patient")[, "Cell"])), ]
}
```
...
```{r data_processing_re-normalisation}
segcopy_filter <- readRDS("segcopy_filter.RDS")
if (gender == "female") {
  segnorm_stats <- readRDS("segnorm_stats_female.RDS")
} else {
  segnorm_stats <- readRDS("segnorm_stats.RDS")
}
segrenorm_data <- segnorm_data[segcopy_filter$filter == 2, ]
segrenorm_data <- cbind(segrenorm_data[,c(1:3)], apply(segrenorm_data[, -c(1:3)], 2, function(x) { x / segnorm_stats$average}))
...
# =============================================================================
manually_filtered_cells <- c("VT15_D702_D506", "VT15_D703_D506", "VT15_D703_D507", "VT15_D709_D504",
                             ...
                             "VT38_N705_S507", "VT43_N702_S508", "VT43_N711_S508", "VT43_N715_S508", 
                             "VT43_N710_S502", "VT42_N707_S510", "VT42_N714_S508", "VT42_N701_S503", 
                             "VT39_N711_S507", "VT39_N710_S510", "VT39_N710_S507", "VT39_N705_S510", 
                             "VT39_N715_S505", "VT46_N704_S510","VT44_N702_S511")
...
if (length(bad_cells) > 0) {
  ...
  segrenorm_data <- segrenorm_data[, -c(which(colnames(segrenorm_data) %in% bad_cells))]
...
# row.names(ploidy_normal) <- ploidy_normal$Sample
# ploidy_normal <- ploidy_normal[-1, ]
...
# segrenorm_data <- cbind(segrenorm_data[,c(1:3)], segrenorm_data[, c(which(colnames(segnorm_data) %in% row.names(ploidy_normal)))])

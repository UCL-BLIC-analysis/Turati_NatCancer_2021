...
single_seg_renorm <- readRDS("cnv_copynumber_Pt3_dia_100k.single_renorm.RDS")
multi_seg_renorm <- readRDS("cnv_copynumber_Pt3_dia_100k.multi_seg_renorm.RDS")
library(xlsx)
meta_data <- read.xlsx(paste0("../data/Samples.xlsx"), sheetIndex = 1)
cells <- meta_data %>% filter(Day == "Diagnosis" & Patient == "Pt3") %>% pull(Cell) %>% as.character()
...
curated_cnv_table <- read_csv("curated_cnv_table.Pt3_dia.csv")
...
```{r get_signal_matrix}
data_env <- new.env()
load("data_Pt3_dia_100k.rda", envir = data_env)
...
* 17 and 18 are merged onto 17
* 18 and 19 are merged onto 19
* 18 is removed

```{r eval = F}
rm(list = ls())
load(Sys.glob("resampling_Pt3__100k_cache/html/define_cnvs_*.RData"))
...
# Merge (16;17) and (17;18) [17 disappears]
cnv_table[16, "end"] <- cnv_table[17, "end"]
cnv_table[18, "start"] <- cnv_table[17, "start"]
cnv_table <- cnv_table[-17, ]
resampled_segcopy_data <- resampled_segcopy_data[, -17]

# cnv_table <- cnv_table[-c(5,25), ]
# resampled_segcopy_data <- resampled_segcopy_data[, -c(5,25)]
event_sizes <- cnv_table$end - cnv_table$start + 1
tree.arcs <- get_tree_from_data_matrix(
  resampled_segcopy_data, plot = T, event_sizes = event_sizes)
```
...
```{r cosmic_annotations_v90, results = "asis", cache = FALSE}
```
...
save.image("image.resampling_Pt3_dia_100k.RData")
load("image.resampling_Pt3_dia_100k.RData")

---
title: "Pt1 diagnosis resampling analysis"
...
single_seg_renorm <- readRDS("cnv_copynumber_Pt1_dia_100k.single_renorm.RDS")
multi_seg_renorm <- readRDS("cnv_copynumber_Pt1_dia_100k.multi_seg_renorm.RDS")
library(xlsx)
meta_data <- read.xlsx(paste0("../data/Samples.xlsx"), sheetIndex = 1)
cells <- meta_data %>% filter(Day == "Diagnosis" & Patient == "Pt1") %>% pull(Cell) %>% as.character()
...
curated_cnv_table <- read_csv("curated_cnv_table.Pt1_dia.csv")
...
```{r get_signal_matrix}
data_env <- new.env()
load("data_Pt1_dia_100k.rda", envir = data_env)
...
```{r cosmic_annotations_v90, results = "asis"}
```
...
plot_cna_tree(tree, event_labels = event_labels, y_top = 19, title = tree_title,
              highlight = list("#DDDD77" = c(4),
                               "#DDDD77" = c(2,3,5,6,7,9,11,12,13,15,16,17),
                               "#EEAABB" = c(8),
                               "#EEBBAA" = c(14)))

plot_cna_tree(tree, event_labels = event_labels, y_top = 19, title = tree_title, show_cna_matrix = F,
              color_by_clone = F, show_parallel_events = F, show_event_labels = T,
              label_size = 2,
              highlight = list("#DDDD99" = c(4),
                               "#DDDD99" = c(2,3,5,6,7,9,11,12,13,15,16,17),
                               "#DDAA99" = c(8),
                               "#DD99DD" = c(14))) +
  geom_label(x = 2, y = 3, label = "1721", col = "#DDDD99") +
  geom_label(x = 2, y = 2, label = "971", col = "#DDAA99") +
  geom_label(x = 2, y = 1, label = "1726", col = "#DD99DD") +
  geom_rect(aes(xmin = 1, xmax = 3, ymin = 2.1, ymax = 2.9), fill = "#DDDD99") +
  geom_rect(aes(xmin = 1, xmax = 3, ymin = 1.1, ymax = 1.9), fill = "#DDAA99") +
  geom_rect(aes(xmin = 1, xmax = 3, ymin = 0.1, ymax = 0.9), fill = "#DD99DD") +
  geom_label(aes(x = 2, y = 2.5, label = "1721"), size = 5) +
  geom_label(aes(x = 2, y = 1.5, label = "971"), size = 5) +
  geom_label(aes(x = 2, y = 0.5, label = "1726"), size = 5)
...
save.image("image.resampling_Pt1_dia_100k.RData")
load("image.resampling_Pt1_dia_100k.RData")
plot_cna_tree(tree, event_labels = event_labels, y_top = 19, title = tree_title, show_cna_matrix = F)
plot_cna_tree(tree, event_labels = event_labels, y_top = 19, title = tree_title, show_cna_matrix = F,
              highlight = list("#DDDD77" = c(4),
                               "#DDDD77" = c(2,3,5,6,7,9,11,12,13,15,16,17),

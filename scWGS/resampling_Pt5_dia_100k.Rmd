...
single_seg_renorm <- readRDS("cnv_copynumber_Pt5_dia_100k.single_seg_renorm_diagnositconly.RDS")
multi_seg_renorm <- readRDS("cnv_copynumber_Pt5_dia_100k.multi_seg_renorm_diagnosticonly.RDS")
library(xlsx)
meta_data <- read.xlsx(paste0("../data/Samples.xlsx"), sheetIndex = 1)
cells <- meta_data %>% filter(Day == "Diagnosis" & Patient == "Pt5") %>% pull(Cell) %>% as.character()
...
curated_cnv_table <- read_csv("curated_cnv_table.Pt5_dia.csv")
...
```{r get_signal_matrix}
data_env <- new.env()
load("data_Pt5_dia_100k_diagnosticonly.rda", envir = data_env)
...
# Merge independent consecutive events

* **1,2,3 happen in one cell in an independent lineage where 2 and 3 happen without 1 => Merge 1,2,3 into 1 for the cell harboring 1**

* **20,21 happen in two cells in an independent lineage where 21 only happens => Merge 20,21 into 20 for the cells harboring 20**

* **22,23 happen in two cells in an independent lineage where 23 only happens => Merge 22,23 into 22 for the cells harboring 22**


```{r merge_consecutive_events}
# Merge 1,2,3 into 1
cells_with_1_2_3 <- which(resampled_segcopy_data[, 1] == 1 & resampled_segcopy_data[, 2] == 1 & resampled_segcopy_data[, 3] == 1)
cells_with_1_only <- which(resampled_segcopy_data[, 1] == 1 & (resampled_segcopy_data[, 2] == 2 | resampled_segcopy_data[, 3] == 2))
if (length(cells_with_1_only) > 0) {
  stop("Error in the assumption to merge 1,2,3 => 1")
}
cnv_table[1, "end"] <- cnv_table[3, "end"]
resampled_segcopy_data[cells_with_1_2_3, c(2, 3)] <- 2


# Merge 20,21 into 20
cells_with_20_21 <- which(resampled_segcopy_data[, 20] == 1 & resampled_segcopy_data[, 21] == 1)
cells_with_20_only <- which(resampled_segcopy_data[, 20] == 1 & resampled_segcopy_data[, 21] == 2)
if (length(cells_with_20_only) > 0) {
  stop("Error in the assumption to merge 20,21 => 20")
}
cnv_table[20, "end"] <- cnv_table[21, "end"]
resampled_segcopy_data[cells_with_20_21, c(21)] <- 2

# Merge 22,23 into 22
cells_with_22_23 <- which(resampled_segcopy_data[, 22] == 3 & resampled_segcopy_data[, 23] == 3)
cells_with_22_only <- which(resampled_segcopy_data[, 22] == 3 & resampled_segcopy_data[, 23] == 2)
if (length(cells_with_22_only) > 0) {
  stop("Error in the assumption to merge 22,23 => 22")
}
cnv_table[22, "end"] <- cnv_table[23, "end"]
resampled_segcopy_data[cells_with_22_23, c(23)] <- 2

# Rebuild the tree based on the modified events
event_sizes <- cnv_table$end - cnv_table$start + 1
tree.arcs <- get_tree_from_data_matrix(
  resampled_segcopy_data, plot = T, event_sizes = event_sizes)
```
...
save.image("image.resampling_Pt5_dia_100k.RData")
load("image.resampling_Pt5_dia_100k.RData")
